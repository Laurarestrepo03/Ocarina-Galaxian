...
def system_update_score(world:esper.World, interface_info:dict, enemies_info, self):        
    ...
    for _, (c_l) in level_component:
        if c_l.enemy_destroyed is not None:
            for score, (c_s, _) in score_components:
                ...
                c_l.score += points
                color = pygame.Color(interface_info["score_value"]["color"]["r"], interface_info["score_value"]["color"]["b"], interface_info["score_value"]["color"]["g"])
